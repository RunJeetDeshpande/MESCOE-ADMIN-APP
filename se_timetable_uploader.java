...
public class se_timetable_uploader extends AppCompatActivity {
...
//    private ProgressDialog pd;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.se_timetable_uploader_activity);
        ...
        comp2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                databaseReference = FirebaseDatabase.getInstance().getReference("SE COMP 2");
                if (pdfData == null) {
                    Toast.makeText(getApplicationContext(), "Please select pdf file", Toast.LENGTH_SHORT).show();
                } else {
                    uploadPDFfile("SE COMP 2/");
                }
            }
        ...
        compss.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                databaseReference = FirebaseDatabase.getInstance().getReference("SE COMP SS");
                if (pdfData == null) {
                    Toast.makeText(getApplicationContext(), "Please select pdf file", Toast.LENGTH_SHORT).show();
                } else {
                    uploadPDFfile("SE COMP 3/");
                }
            }
...
//                        float percent = (100 * taskSnapshot.getBytesTransferred()) / taskSnapshot.getTotalByteCount();
    ...
    private void selectPDF() {
        ...
        intent.setType("pdf/docs/ppt/images");
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQ && resultCode == RESULT_OK && data != null && data.getData() != null) {
            ...
            if (pdfData.toString().startsWith("content://")) {
                Cursor cursor = null;
                try {
                    cursor = se_timetable_uploader.this.getContentResolver().query(pdfData, null, null, null, null);
